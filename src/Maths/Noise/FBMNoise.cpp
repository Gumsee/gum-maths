...
namespace Noise {
    ...
    float fbmPerlin(vec2 pos, vec2 scale, int octaves, float shift, float axialShift, float gain, float lacunarity, uint mode, float factor, float offset, float octaveFactor, float seed) 
    {
        ...
        float n = 1.0;
        vec2 p = vec2::fract(pos) * frequency;
    ...
    float fbmGridt(vec2 pos, vec2 scale, int octaves, float shift, float timeShift, float gain, float lacunarity, vec3 translate, float warpStrength, float octaveFactor, float seed) 
    {
        ...
        for (int i = 0; i < octaves; i++) 
        {
            ...
            vec4 mn = multiNoise(vec4(pi.x, pi.y, pi.x + translate.x, pi.y + translate.x), vec4(frequency.x, frequency.y, frequency.x, frequency.y), time, seed);
            vec2 mn0 = multiNoise(vec4(pi.x + translate.y, pi.y + translate.y, pi.x + translate.z, pi.y + translate.z), vec4(frequency.x, frequency.y, frequency.x, frequency.y), time, seed);
            mn.z = mn0.x;
            mn.w = mn0.y;
            mn.x = mn.x * mn.z;
            mn.y = mn.y * mn.w;
    ...
    {    
        ...
        for (int i = 0; i < octaves; i++) 
        {
            ...
            vec4 n = multiNoise(vec4(pos.x, pos.y, pos.x, pos.y), vec4(ns.x, ns.y, ns.x, ns.y), phase, vec2(seeds.x, seeds.y));
